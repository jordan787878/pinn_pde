...
def show_p_net_results(p_net):
    ...
    #     pt_t = Variable(torch.from_numpy(x*0+t1).float(), requires_grad=True).to(device)
...
def show_e1_net_results(p_net, e1_net):
    ...
    for i, (t1, ax1, e1, e1_hat, alpha_1) in enumerate(zip(t1s, axs, e1_list, e1_hat_list, alpha_1_list)):
        ...
        if i == 0:
            ...
            ax1.fill_between(x.reshape(-1), y1=0*x.reshape(-1)+error_bound, y2=0*x.reshape(-1)-error_bound, color="green", alpha=0.3, label="Error Bound(t)")
        ax1.set_ylim([-1.1*error_bound, 1.1*error_bound])
...
# 
# best epoch:  178200 , loss: tensor(8.3387e-05)
# t1= 1.0 , a1 [Monte]= [0.01918764]
# t1= 2.0 , a1 [Monte]= [0.82774691]
# t1= 3.0 , a1 [Monte]= [1.06495371]
# t1= 4.0 , a1 [Monte]= [1.61851823]
# t1= 5.0 , a1 [Monte]= [1.87770436]
# t1= 6.0 , a1 [Monte]= [1.98331727]