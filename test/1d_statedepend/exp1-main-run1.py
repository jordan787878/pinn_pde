...
def train_p_net(p_net, optimizer, scheduler, mse_cost_function, max_abs_p_ti, iterations=40000):
    ...
    x_bc = (torch.rand(500, 1)*(x_hig-x_low)+x_low).to(device)
    ...
    for epoch in range(iterations):
        ...
        res_out = res_func(x, t, p_net, verbose=False)/max_abs_p_ti
...
def show_p_net_results(p_net):
    ...
    for i, (ax1, e1) in enumerate(zip(axs, e1_list)):
        if i == 0:
            ax1.plot(x, e1_true, "black", linestyle="-", label=r"$e_1$")
            ax1.plot(x, e1, "blue", linestyle="-", label=r"$e_1$ [Monte]")
            ax1.legend()
        else:
