...
def check_pnn_result(p_net):
    global constants
    x1s, x2s, x3s, x1_grid, x2_grid, x3_grid, grid_points = constants.prepare_gridpoints()
    for t in constants.T_SPAN:
        # print("[check] grid points shape type: ", grid_points.shape, grid_points.dtype)
        # compute p(true)
        pdf_true = constants.p_sol(grid_points, t).reshape(x1_grid.shape)
...
def check_e1nn_result(e1_net, p_net):
    ...
    for t in constants.T_SPAN:
        ...
        print( " =result= e_nn(t={:.1f}) error: {:.4f}, eS: {:.4f}, a1: {:.3f}".format(t, np.max(np.abs(e2)), eS, a1) )
