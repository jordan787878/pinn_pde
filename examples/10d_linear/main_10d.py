...
import time
from constants import MyConstants10D
import argparse
from torch.utils.data import Dataset, DataLoader


TRAIN_FLAG = False
constants = MyConstants10D() #(testing non singular 5D A, will turn to 7D later)
...
def dyn_f8(x):
    ...
    result = torch.zeros(x.shape[0])
    for i in range(constants.DIM):
        result = result + constants.A_TENSOR[7,i]*x[:,i]
    return result
...
def dyn_f9(x):
    ...
    result = torch.zeros(x.shape[0])
    for i in range(constants.DIM):
        result = result + constants.A_TENSOR[8,i]*x[:,i]
    return result
...
def dyn_f10(x):
    ...
    result = torch.zeros(x.shape[0])
    for i in range(constants.DIM):
        result = result + constants.A_TENSOR[9,i]*x[:,i]
    return result
...
def diff_opt(x, t, net, verbose=False):
    ...
    f8 = dyn_f8(x).view(-1,1)
    f9 = dyn_f9(x).view(-1,1)
    f10 = dyn_f10(x).view(-1,1)
    fs = torch.cat((f1, f2, f3, f4, f5, f6, f7, f8, f9, f10), dim=1)
...
def train_p_net(p_net, optimizer, scheduler, mse_cost_function, iterations=40000):
    ...
    # samples of initial condition
    _x_bc_normal = np.random.multivariate_normal(constants.N_MEAN_I, constants.N_COV_I, size=init_samples).astype(np.float32)
    _x_bc_normal = torch.tensor(_x_bc_normal, dtype=torch.float32, requires_grad=False)
    _x_bc = np.column_stack([
        np.random.uniform(constants.X1_RANGE[0], constants.X1_RANGE[1], init_samples),
        np.random.uniform(constants.X2_RANGE[0], constants.X2_RANGE[1], init_samples),
        np.random.uniform(constants.X3_RANGE[0], constants.X3_RANGE[1], init_samples),
        np.random.uniform(constants.X4_RANGE[0], constants.X4_RANGE[1], init_samples),
    ])
    _x_bc = torch.tensor(_x_bc, dtype=torch.float32, requires_grad=False)
    x_bc = torch.cat((_x_bc_normal, _x_bc), dim=0)
    ...
    # samples of residual
    _x_normal = np.random.multivariate_normal(constants.N_MEAN_I, constants.N_COV_I, size=init_samples).astype(np.float32)
    _x_normal = torch.tensor(_x_normal, dtype=torch.float32, requires_grad=True)
    _x = np.column_stack([
        np.random.uniform(constants.X1_RANGE[0], constants.X1_RANGE[1], init_samples),
        np.random.uniform(constants.X2_RANGE[0], constants.X2_RANGE[1], init_samples),
    ...
    for epoch in range(iterations):
        ...
        if(epoch > 0):
            ...
            # mse_res_grad = torch.mean((res_x**2).sum(dim=1))
            loss_dataset = mse_u + (constants.TF/constants.T)*(mse_res)
            ...
            if(FLAG):
                ...
                _x_bc_normal = np.random.multivariate_normal(constants.N_MEAN_I, constants.N_COV_I, size=S).astype(np.float32)
                _x_bc_normal = torch.tensor(_x_bc_normal, dtype=torch.float32, requires_grad=False)
                _x_bc = np.column_stack([
                    np.random.uniform(constants.X1_RANGE[0], constants.X1_RANGE[1], S),
                    np.random.uniform(constants.X2_RANGE[0], constants.X2_RANGE[1], S),
                    np.random.uniform(constants.X3_RANGE[0], constants.X3_RANGE[1], S),
                    np.random.uniform(constants.X4_RANGE[0], constants.X4_RANGE[1], S),
                ])
                _x_bc = torch.tensor(_x_bc, dtype=torch.float32, requires_grad=False)
                x_bc_rar = torch.cat((_x_bc_normal, _x_bc), dim=0)
                t_bc_rar = (torch.ones(len(x_bc_rar), 1) * constants.TI).to(device)

                _x_normal = np.random.multivariate_normal(constants.N_MEAN_I, constants.N_COV_I, size=S).astype(np.float32)
                _x_normal = torch.tensor(_x_normal, dtype=torch.float32, requires_grad=True)
                _x = np.column_stack([
                    np.random.uniform(constants.X1_RANGE[0], constants.X1_RANGE[1], S),
                    np.random.uniform(constants.X2_RANGE[0], constants.X2_RANGE[1], S),
                ...
                _x = torch.tensor(_x, dtype=torch.float32, requires_grad=True)
                x_rar = torch.cat((_x_normal, _x), dim=0)
                t_rar = np.random.uniform(constants.TI, constants.TF/constants.T, len(x_rar)),
                t_rar = torch.tensor(t_rar, dtype=torch.float32, requires_grad=True).view(-1,1)
                # add ic points
        ...
        for iter, (x_bc_k, t_bc_k, x_k, t_k) in enumerate(data_loader, start=1): 
            ...
            # Loss based on boundary conditions
            p_i = p_init(x_bc_k.detach().numpy())
            p_i = torch.tensor(p_i, dtype=torch.float32, requires_grad=False)
            phat_i = p_net(x_bc_k, t_bc_k).to(device)
            mse_u = mse_cost_function(phat_i/normalize, p_i/normalize)

            # Loss based on PDE
            res_p = diff_opt_p(x_k, t_k, p_net)/normalize
            all_zeros = torch.zeros((len(t_k),1), dtype=torch.float32, requires_grad=False).to(device)
            mse_res = mse_cost_function(res_p, all_zeros)
            ...
            # # res_grad = torch.sum(res_grad**2, dim=1, keepdim=True)
            # mse_res_grad = torch.mean((res_x**2).sum(dim=1))
            ...
            #     train_time = time.time() - start_time
            #     # Print the iteration number (and optionally the loss)
            #     print(f'Epoch [{epoch+1}/{iterations}], Iteration [{iter}/{len(data_loader)}], Loss: {loss.item():.4f}, ic: {mse_u.item():.4f}, res: {mse_res.item():.4f}')
            #     torch.save({
            #             'epoch': epoch,
            #             'model_state_dict': p_net.state_dict(),
            #             'optimizer_state_dict': optimizer.state_dict(),
...
def main():
    ...
    if(TRAIN_FLAG):
        train_p_net(p_net, optimizer, scheduler, mse_cost_function, iterations=200000); print("p_net train complete")
