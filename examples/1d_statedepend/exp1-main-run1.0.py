...
def train_p_net(p_net, optimizer, scheduler, mse_cost_function, p0_max, iterations=40000):
    min_loss = np.inf
    loss_history = []
    ...
    N_ic = 250
    x_bc = (torch.rand(N_ic, 1)*(x_hig-x_low)+x_low).to(device)
    x_bc_normal = mvn.sample((N_ic,)).to(device)
    ...
    N_res = 1000
    t = (torch.rand(N_res, 1, requires_grad=True)*(T_end-t0)   +t0).to(device)
    t_init = (torch.ones(int(0.1*N_res), 1, requires_grad=True) * t0).to(device)
    ...
    for epoch in range(iterations):
        ...
        mse_u = mse_cost_function(net_bc_out/p0_max, u_bc/p0_max)
        ...
        res_out = res_func(x, t, p_net, verbose=False)/p0_max
        ...
        if (epoch%1000 == 0 and FLAG):
            ...
            res_RAR = res_func(x_RAR, t_RAR, p_net)/p0_max
...
def show_e1_net_results(p_net, e1_net):
    ...
    for i, (t1, ax1, e1, e1_hat, alpha_1) in enumerate(zip(t1s, axs, e1_list, e1_hat_list, alpha_1_list)):
        ...
        ax1.set_ylim([-2.0*error_bound, 2.0*error_bound])
    ...
    #     if i == 0:
    #         ax1.plot(x, p, "black", linewidth=1.0, label=r"$p$")
    #         ax1.plot(x, p_hat, "blue", linewidth=1.0, linestyle=lstyles[i], label=r"$\hat{p}$")
    #         # ax1.plot(x, e1_hat, "red", linestyle="--", label=r"$\hat{e}_1$")
    #         ax1.fill_between(x.reshape(-1), y1=p_hat.reshape(-1)+error_bound, y2=p_hat.reshape(-1)-error_bound, color="green", alpha=0.2, label=r"$e_L$")
    #         ax1.legend(loc="upper right")
    #     else:
    #         ax1.plot(x, p, "black", linewidth=1.0)
    #         ax1.plot(x, p_hat, "blue", linewidth=1.0, linestyle=lstyles[i])
    #         ax1.fill_between(x.reshape(-1), y1=p_hat.reshape(-1)+error_bound, y2=p_hat.reshape(-1)-error_bound, color="green", alpha=0.2)
    # ax1.grid(True, which='both', linestyle='-', linewidth=0.5)  # Add thin grid lines
    # # Add text to the left top corner
...
def main():
    ...
    p0_max = get_p0_max()
    print("p0_max: ", p0_max)
    train_p_net(p_net, optimizer, scheduler, mse_cost_function, p0_max, iterations=50000); print("[p_net train complete]")
