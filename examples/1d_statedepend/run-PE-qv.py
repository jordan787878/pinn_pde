...
def train_model(net, optimizer, scheduler, mse_cost_function, iterations=40000):
    ...
    for epoch in range(iterations):
        ...
        # loss
        # loss = mse_ic + mse_res_p + mse_p_res_grad + mse_ic_e+  mse_res_e + mse_e_res_grad
...
def show_net_results(net):
    ...
    x = np.linspace(x_low, x_hig, num=200).reshape(-1,1)
    ...
    fig, axs = plt.subplots(3, 2, figsize=(8, 6))
    for i, (e1, e1_hat, p_monte) in enumerate(zip(e1_list, e1_hat_list, p_monte_list)):
        eL = 2.0 * np.max(np.abs(e1_hat))
        if i == 0:
            ax1 = axs[0,0]
        if i == 1:
        ...
        if i == 5:
            ax1 = axs[2,1]
        print("a1: ", np.round( np.max(np.abs(e1-e1_hat))/np.max(np.abs(e1_hat)), 3), 
              ", eL (%): ", np.round( 100.0*eL/np.max(np.abs(p_monte)), 2))
        arg_max_e = np.argmax(np.abs(e1-e1_hat))
        arg_max_e1hat = np.argmax(np.abs(e1_hat))
        if i == 0:
            ...
            ax1.fill_between(x.reshape(-1), y1=0*x.reshape(-1)+eL, y2=0*x.reshape(-1)-eL, color="green", alpha=0.2)
        # ax1.scatter(x[arg_max_e], e1[arg_max_e], color="blue")
        ax1.plot([x[arg_max_e], x[arg_max_e]], [e1[arg_max_e], e1_hat[arg_max_e]], color="blue", linestyle=":", linewidth=1.0)
        #ax1.scatter(x[arg_max_e1hat], e1_hat[arg_max_e1hat], color="red")
        ax1.plot([x[arg_max_e1hat], x[arg_max_e1hat]], [e1_hat[arg_max_e1hat]*0, e1_hat[arg_max_e1hat]], color="red", linestyle=":", linewidth=1.0)
        # ax1.plot([x[arg_max_e1hat], x[arg_max_e1hat]], [0, e1_hat[arg_max_e1hat]], color="red", linestyle=":", linewidth=1.0)
        ax1.plot([x_low, x_hig], [0, 0], color="black", linestyle=":", linewidth=0.5)
        ax1.grid(True, which='both', linestyle='-', linewidth=0.5)
    plt.tight_layout()
...
def plot_p_surface(net, num=100):
    ...
    phat = net(pt_x, pt_t)[:, 0].data.cpu().numpy().reshape(num, -1)

    p_list = []
    for t1 in t1s:
        p_true = p_sol(x, t1).reshape(-1)
        p_list.append(p_true)

    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    ax.plot_surface(x_mesh, t_mesh, phat, cmap='viridis', alpha=0.7)
    ...
    for i in range(len(t1s)):
        t1 = t1s[i]
        ax.plot(x, t1, p_list[i], color="black")
...
def plot_e_surface(net, num=100):
    x = np.linspace(x_low, x_hig, num=num)
    t = np.linspace(t0, T_end, num=num)
    x_mesh, t_mesh = np.meshgrid(x,t)
    pt_x = Variable(torch.from_numpy(x_mesh.reshape(-1,1)).float(), requires_grad=True).to(device)
    pt_t = Variable(torch.from_numpy(t_mesh.reshape(-1,1)).float(), requires_grad=True).to(device)
    ehat = net(pt_x, pt_t)[:, 1].data.cpu().numpy().reshape(num, -1)

    e_list = []
    for t1 in t1s:
        p_true = p_sol(x, t1).reshape(-1)
        x_col = Variable(torch.from_numpy(x.reshape(-1,1)).float(), requires_grad=True).to(device)
        t_col = x_col*0 + t1
        p_hat = net(x_col, t_col)[:, 0].data.cpu().numpy().reshape(-1)
        e = p_true - p_hat
        e_list.append(e)
    ...
    ax.plot_surface(x_mesh, t_mesh, ehat, cmap='viridis', alpha=0.7)
    ...
    for i in range(len(t1s)):
        t1 = t1s[i]
        ax.plot(x, t1, e_list[i], color="black")
...
def main():
    ...
    # Visualize Surface (X,T)

    # plot_p_surface(model)

    # plot_e_surface(model)
