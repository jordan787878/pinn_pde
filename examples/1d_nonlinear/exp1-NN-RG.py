...
# Log
# enet best epoch:  46124 , loss: tensor(0.0030)
# eL:  0.028 	 alpha:  0.226
# eL:  0.033 	 alpha:  0.438
# eL:  0.038 	 alpha:  0.737
# eL:  0.04 	 alpha:  0.896
# eL:  0.041 	 alpha:  0.951
# eL:  0.04 	 alpha:  0.957
# train pnet time: 598.7061 seconds
# train enet time: 2033.9315 seconds

FOLDER = "exp1/exp1-NN-RG/"
...
def train_pnet_model(net, optimizer, scheduler, mse_cost_function, iterations=40000):
    ...
    for epoch in range(iterations):
        ...
        if(loss.data < pnet_loss_termination):
            print("epoch:", epoch, ",loss:", loss.data, 
                  ",ic:",mse_ic_p.data, 
                  ",res:", mse_res_p.data,
                  ",res g:", mse_res_p_g.data,
...
def main():
    ...
    train_pnet_model(p_model, optimizer_p_model, scheduler_p_model, mse_cost_function, iterations=50000); print("[pnet train complete]")
    end_time = time.time()
    time_train_p = end_time - start_time
    p_model = load_trained_model(p_model, PATH=FOLDER+"output/pnet.pt", PATH_LOSS=FOLDER+"output/pnet_train_loss.npy")
    p_model.eval()
    ...
    start_time = time.time()
    train_enet_model(p_model, e_model, optimizer_e_model, scheduler_e_model, mse_cost_function, iterations=50000); print("[enet train complete]")
    end_time = time.time()
    time_train_e = end_time - start_time
    e_model = load_trained_model(e_model, PATH=FOLDER+"output/enet.pt", PATH_LOSS=FOLDER+"output/enet_train_loss.npy")
    e_model.eval()
