...
def p_res_func(x, t, net, verbose=False):
    p = net(x, t)[:, 0].view(-1, 1)
...
def e_res_func(x, t, net, verbose=False):
    e1_out = net(x, t)[:, 1].view(-1, 1)
    p_res = p_res_func(x, t, net)
...
def e2_res_func(x, t, net, verbose=False):
    e2_out = net(x, t)[:, 2].view(-1, 1)
    e1_res = e_res_func(x, t, net)
    e_x = torch.autograd.grad(e2_out, x, grad_outputs=torch.ones_like(e2_out), create_graph=True)[0]
    e_t = torch.autograd.grad(e2_out, t, grad_outputs=torch.ones_like(e2_out), create_graph=True)[0]
    e_xx = torch.autograd.grad(e_x,  x, grad_outputs=torch.ones_like(e_x), create_graph=True)[0]
    residual = e1_res + e_t + (3*const_a*x*x + 2*const_b*x + const_c)*e2_out \
                     + (const_a*x*x*x + const_b*x*x + const_c*x + const_d)*e_x \
                     - 0.5*const_e*const_e*e_xx
    if(verbose):
        print(e_x.shape, e_t.shape, e_xx.shape, residual.shape)
...
def train_model(net, optimizer, scheduler, mse_cost_function, iterations=40000):
    ...
    for epoch in range(iterations):
        ...
        # e init loss
        normalize = 1.0 #torch.max(torch.abs(p0-p0_hat), dim=0)[0].detach()
        ...
        e_res = e_res_func(x, t, net)
        mse_res_e = mse_cost_function(e_res/normalize, all_zeros)
        ...
        normalize = 1.0# torch.max(torch.abs(e0_true-e0_hat), dim=0)[0].detach()
        e20_hat = net_out_0[:, 2].reshape(-1,1)
        e20_true = (e0_true - e0_hat).detach().numpy()
        e20_true = Variable(torch.from_numpy(e20_true).float(), requires_grad=False).to(device)
        mse_ic_e2 = mse_cost_function(e20_true/normalize, e20_hat/normalize)
        e2_res = e2_res_func(x, t, net)
        mse_res_e2 = mse_cost_function(e2_res/normalize, all_zeros)
        _res_x = torch.autograd.grad((e2_res), x, grad_outputs=torch.ones_like(e2_res), create_graph=True)[0]
        _res_t = torch.autograd.grad((e2_res), t, grad_outputs=torch.ones_like(e2_res), create_graph=True)[0]
        mse_res_e2_g = torch.mean(_res_x**2 + _res_t**2)

        # metric
        e1_hat = net(x, t)[:, 1].reshape(-1,1)
        e2_hat = net(x, t)[:, 2].reshape(-1,1)
        metric = 2*torch.max(torch.abs(e2_hat))/ torch.mean(torch.abs(e1_hat))
        
        
        loss = (mse_ic_p + mse_res_p + mse_res_p_g) + 10*(mse_ic_e + mse_res_e + mse_res_e_g) + 10*(mse_ic_e2 + mse_res_e2 + mse_res_e2_g)
        # w_p = lambda_l0*pow((1-a_l0),(epoch/T_l0))
        # w_e = lambda_l1/(1+np.exp(-a_l1*(epoch-T_l1)))
        # loss = w_p * (mse_ic_p + mse_res_p + mse_res_p_g) + w_e * (mse_ic_e + mse_res_e + mse_res_e_g)
        ...
        if(loss.data < 0.9*min_loss):
            print("epoch:", epoch, ",loss:", loss.data, 
                  ...
                  "  ,ic e2:",mse_ic_e2.data, 
                  ",res e2:", mse_res_e2.data,
                  ",res g e2:", mse_res_e2_g.data,
                  )
            print(metric.data)
            torch.save({
                    'epoch': epoch,
        ...
        if ((epoch+1)%1000==0 and FLAG >= 1):
            ...
            p_res_RAR = p_res_func(x_RAR, t_RAR, net)
            e_res_RAR = e_res_func(x_RAR, t_RAR, net)
            e2_res_RAR = e2_res_func(x_RAR, t_RAR, net)
            ...
            # add to e2 residual samples
            max_abs_res, max_index = torch.topk(torch.abs(e2_res_RAR), 3, dim=0)
            x_max = x_RAR[max_index].reshape(-1,1)
            t_max = t_RAR[max_index].reshape(-1,1)
            x = torch.cat((x, x_max), dim=0)
            t = torch.cat((t, t_max), dim=0)
...
def show_net_results(net):
    ...
    for t1 in t1s:
        ...
        pres = p_res_func(pt_x, pt_t1, net).reshape(-1,1).detach().numpy()
        eres = e_res_func(pt_x, pt_t1, net).reshape(-1,1).detach().numpy()
    ...
    for i, (e1, e1_hat) in enumerate(zip(e1_list, e1_hat_list)):
        ...
        if i == 5:
            ax1 = axs[2,1]
        a1 = np.max(np.abs(e1-e1_hat))/np.max(np.abs(e1_hat))
        a1 = np.round(a1, 2)
        print("eL: ", np.round(eL,3), ", a1: ", a1)
        if i == 0:
    ...
    fig, axs = plt.subplots(3, 2, figsize=(8, 6))
    for i, (e2, e2_hat) in enumerate(zip(e2_list, e2_hat_list)):
        if i == 0:
            ax1 = axs[0,0]
        if i == 1:
            ax1 = axs[1,0]
        ...
        if i == 5:
            ax1 = axs[2,1]
        if i == 0:
            ax1.plot(x, e2, "blue", label=r"$e_2$")
            ax1.plot(x, e2_hat, "red", linestyle="--", label=r"$\hat{e}_2$")
            ax1.legend()  # Add legend only to the first subplot
        else:
            ax1.plot(x, e2, "blue")
            ax1.plot(x, e2_hat, "red", linestyle="--")
        ax1.grid(True, which='both', linestyle='-', linewidth=0.5)
    plt.tight_layout()
    plt.savefig(FOLDER+"figs/net_result_3.png")
    plt.close()

    fig, axs = plt.subplots(3, 2, figsize=(8, 6))
    for i, (pres, eres) in enumerate(zip(pres_list, pres_list)):
        if i == 0:
            ax1 = axs[0,0]
        if i == 1:
            ax1 = axs[1,0]
        ...
        if i == 5:
            ax1 = axs[2,1]
        ax1.plot(x, -pres, "black", label=r"$-r1$")
        ax1.plot(x, eres-pres, "red", linestyle="--", label=r"$D[\hat{e}_1]$")
        ax1.legend()  # Add legend only to the first subplot
        ax1.grid(True, which='both', linestyle='-', linewidth=0.5)
    plt.tight_layout()
    plt.savefig(FOLDER+"figs/net_residuals.png")
    plt.close()
