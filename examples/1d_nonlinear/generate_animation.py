...
from main_stat import x_low, x_hig, t0, T_end, PNet, load_trained_model, FOLDER, p_init, t1s, DATA_FOLDER, datas, get_e1_normalize, E1Net
...
for t1 in t1s:
    ...
    line_k, = ax.plot(x, p, color="black", linewidth=0.2, alpha=0.5, label=r"$p$" if t1 == 0 else "")
...
line2, = ax.plot(x, p0_hat, color='blue', label=r"$\hat{p}$")
eS = 2*np.max(np.abs(e0_hat))
fill_between_area = ax.fill_between(x.reshape(-1), 
                                    p0_hat.reshape(-1) + eS, 
                                    p0_hat.reshape(-1) - eS, color="green", alpha=0.3, label=r"$B_1$")
ax.set_xlabel('x')
ax.set_ylabel('p(x, t)')
...
ani.save(FOLDER+'figs/error_bound_animation.mp4', writer='ffmpeg', fps=10)
