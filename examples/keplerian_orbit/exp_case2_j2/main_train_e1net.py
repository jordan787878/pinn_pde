...
def train_model(e1_net, p_net, optimizer, scheduler, mse_cost_function, iterations=50000):
    ...
    for epoch in range(iterations):
        ...
        all_zeros = torch.zeros((len(t_bc),1), dtype=torch.float32, requires_grad=False).to(device)
        ic = e_i - ehat_i
        mse_u = mse_cost_function(ic/normalize, all_zeros)
        ...
        #     p_i = torch.tensor(p_i, dtype=torch.float32, requires_grad=False)
        #     phat_i = p_net(z_bc, t_bc).to(device).detach()
        #     e_i = p_i - phat_i
        #     ehat_i = e1_net(z_bc, t_bc).to(device)
        #     ic = e_i - ehat_i
        #     mse_u = mse_u + mse_cost_function(ic/normalize, all_zeros)
        # mse_u = mse_u/(N_jj + 1)
        ...
        mse_res = mse_cost_function(res, all_zeros)
...
def check_train_results(e1_net, p_net, t=0.0):
    ...
    return max_e1
    # indices = np.arange(1, 1+e1_vec.shape[0])
    # plt.figure
    # plt.plot(indices, e1_vec, "black")
    # plt.plot(indices, e1_nn_vec, "blue")
    # plt.show()
    # print(e1_vec)
    # print(e1_nn_vec)
...
def main():
    ...
    if(TRAIN_FLAG):
        train_model(e1_net, p_net, optimizer, scheduler, mse_cost_function, iterations=100000); print("e1_net train complete")
