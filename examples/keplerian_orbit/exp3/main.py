...
mean_motion_fixed = math.sqrt(mu_gravity/a_0**3)
...
def train_p_net(p_net, optimizer, scheduler, mse_cost_function, iterations=40000):
    ...
    x[:,1] += mean_motion_fixed * t.squeeze()
    ...
    for epoch in range(iterations):
        ...
        if (epoch%100 == 0 and FLAG):
            ...
            t0_RAR = 0.0*t_RAR.clone().detach() + ti
            x0_RAR = x_RAR.clone().detach()

            # drift_x2_RAR = math.sqrt(mu_gravity/a_0**3)
            x_RAR[:,1] += mean_motion_fixed * t_RAR.squeeze()

            p_bc_RAR = p_init_torch(x0_RAR)
            phat_bc_RAR = p_net(x0_RAR, t0_RAR)
...
def main():
    ...
    t1s = [ti, 7200.0, 86400.0, tf]
    for t1 in t1s:
        # load data
        x1_grid = np.load(DATA_FOLDER+"x1_grid"+str(t1)+".npy")
        x2_grid = np.load(DATA_FOLDER+"x2_grid"+str(t1)+".npy")
