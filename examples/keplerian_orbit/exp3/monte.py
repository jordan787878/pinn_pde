...
def p_sol_monte(t1=ti, linespace_num=100, stat_sample=100000000):
    mean = mu_0
    cov = cov_0
    X = np.random.multivariate_normal(mean, cov, size=stat_sample).T
    X[1,:] = X[1,:] + t1*np.sqrt(mu_gravity/X[0,:]**3)

    # drift from the mean dynamic motion
    drift_x2 = lam_0 + t1*np.sqrt(mu_gravity/a_0**3)
    
    # Define bins as the edges of x1 and x2
    bins_x1 = np.linspace(x1_low, x1_hig, num=linespace_num, endpoint=True)
    bins_x2 = np.linspace(x2_low+drift_x2, x2_hig+drift_x2, num=linespace_num, endpoint=True)
...
def main():
    t1s = [0.0, 7200.0, 86400.0, 2*86400.0]

    for t1 in t1s:
        x1_grid, x2_grid, pdf = p_sol_monte(t1)
        np.save(DATA_FOLDER+"pdf_t"+str(t1)+".npy", pdf)
        np.save(DATA_FOLDER+"x1_grid"+str(t1)+".npy", x1_grid)
        np.save(DATA_FOLDER+"x2_grid"+str(t1)+".npy", x2_grid)
