...
from utilities._General.neuralnetworks import (PNet_XL, ENet_XL,
        TimeToGMM_V0, TimeToGMM_NoEncoder, TimeToGMM_IC, load_trained_model)
...
def compute_pdf_variations(data_mc=None, p_net=None, p_net_gmm=None, 
    ...
    for idx, t in enumerate(metrics["t"]):
        ...
        if(p_net_gmm_noencoder is not None):
            g_kl_pinngmm_noencoder = compute_generalKL(t, X_mc, p_net=p_net_gmm_noencoder)
...
def main():
    ...
    plot_marginal_pdf_cart(constants, data_mc, _data_folder_no_thrust, p_net_gmm=p_net_gmm)
    plt.show()
