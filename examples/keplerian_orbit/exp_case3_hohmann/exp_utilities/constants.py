...
class Case2_4D_Constants_Low_Thrust:
    ...
    _A_THRUST = np.float32(1e-2)

    # Domain of TF = 0.2*T
    _N_X1_RANGE = np.float32(np.array([17.0, 30.0]))
    _N_X2_RANGE = np.float32(np.array([-8., 8.]))
    _N_X3_RANGE = np.float32(np.array([-15.0, 40.0]))
    _N_X4_RANGE = np.float32(np.array([-55.0, 65.0]))
    
    _T_PRIME_SPAN   = np.round(np.float32(np.array([0.00, 0.06, 0.12, 0.18, 0.21, 0.24, 0.27, 0.30])), 2)
