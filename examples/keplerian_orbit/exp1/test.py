...
def test_4(t1):
    # Change cov[1,1] from 2.92 to 0.92, then the pdf visualization breaks apart
    # However, if we refine the y range from (-6,28) to (14,22), then the pdf visual looks OK
    # This "breaking visual" is due to coarse domain resolution ...

    # mean = [8000, 19]
    mean = [7999.9575, 0.0001]
    # cov = np.array([[62430, -223.636], [-223.636, 0.920]])
    cov = np.array([[62500, 0.0], [0.0, 0.12185]])
    cov = 0.5*(cov + cov.T)
    ...
    plot_multivariate_normal(mean, cov, x_range, y_range)


def plot_multivariate_normal(mean, covariance, x_range, y_range):
    ...
    # Create a 2D array of shape (100, 100) to hold the probabilities
    ...
    pdf_func = multivariate_normal(mean, covariance)
    Z = np.float32(pdf_func.pdf(xs)).reshape(-1,1).reshape(Nx, Nx)

    x1_range = x1_grid[0,:]
    x2_range = x2_grid[:,0]
    dx1 = x1_range[1] - x1_range[0]
    dx2 = x2_range[1] - x2_range[0]
    pdf_sum = np.sum(Z) * dx1 * dx2
    print("[debug] p sum: ", pdf_sum)

    # print(Z.shape)
    # Create the plot
    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(111, projection='3d')
    ax.plot_surface(x1_grid, x2_grid, Z, cmap='viridis', edgecolor='none')
