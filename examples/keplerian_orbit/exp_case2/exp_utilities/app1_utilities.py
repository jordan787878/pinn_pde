...
def compute_prob_event_monte(constants, target_r, targer_ph, t):
    ...
    in_bounds_rows = grid_points[mask_flat]
    # print(in_bounds_rows)
    ...
    for j in range(N_monte):
        ...
        if not os.path.exists(file_name):
            ...
            pdf_mc_grid = np.load(mc_folders[j]+"pdf_t{:.3f}.npy".format(t)).flatten()
            pr = np.sum(pdf_mc_grid[mask_flat]) * dV
            pr_list.append(pr)
    return pr_list
...
def form_problem_exp_case2(problem, need_grid=False):
