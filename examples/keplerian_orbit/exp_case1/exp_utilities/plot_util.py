...
def compute_marginals_over_time(
    ...
    file_exists = np.array([os.path.isfile(os.path.join(pdf_dir, filename_fmt.format(t))) for t in times], dtype=bool)
    times_kept = times[file_exists]
    if times_kept.size == 0:
        raise FileNotFoundError(f"No matching files in '{pdf_dir}' using format '{filename_fmt}'")
    ...
    for j, t in enumerate(times_kept):
        fpath = os.path.join(pdf_dir, filename_fmt.format(t))
        f = np.load(fpath)
        if f.ndim != 6:
...
# ---------- plotting ----------
def plot_time_curves_3d(data1, data2=None, title="Marginal p(x1|t): curves (no interpolation)"):
    """
    ...
    for j, t in enumerate(times):
        ax.plot(np.full_like(x_keep, t), x_keep, M[:, j], lw=1.7)  # x=time, y=x1, z=pdf

    if data2 is not None:
        x_keep = data2["x"]; times = data2["times"]; M = data2["M"]
        for j, t in enumerate(times):
            ax.plot(np.full_like(x_keep, t), x_keep, M[:, j], lw=1.7, linestyle="--")  # x=time, y=x1, z=pdf

    ax.set_xlabel("time t")
    ax.set_ylabel("x")
    ax.set_zlabel("p(x | t)")
    ax.set_title(title)
    # tweak view if desired
