...
DATA_TO_LOAD_META = "meta_data/exp1/"
DATA_TO_SAVE = "meta_data/exp1/data_combine/"
N_data = 1

def main():  

    x1_grid = np.load(DATA_TO_LOAD_META+"x1_grid.npy")
    x2_grid = np.load(DATA_TO_LOAD_META+"x2_grid.npy")
    ...
    for t1 in t1s:
        ...
        for j in range(1, N_data+1):
            pdf = np.load(DATA_TO_LOAD_META+"data_"+str(j)+"/pdf_t"+str(t1)+".npy")
            print("load data: ", DATA_TO_LOAD_META+"data_"+str(j)+"/pdf_t"+str(t1)+".npy")
