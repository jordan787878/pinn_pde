...
class Net(nn.Module):
    def __init__(self, scale=1.0, verbose=False): 
        ...
        self.hidden_layer1 = (nn.Linear(1,neurons))
        self.hidden_layer2 = (nn.Linear(neurons,neurons))
        # self.hidden_layer3 = (nn.Linear(neurons,neurons))
        # self.hidden_layer4 = (nn.Linear(neurons,neurons))
        # self.hidden_layer5 = (nn.Linear(neurons,neurons))
        self.output_layer =  (nn.Linear(neurons,7))
    def forward(self, x, t):
        _x1 = x[:,0].view(-1, 1)
        _x2 = x[:,1].view(-1, 1)
        _t  = t/t_orbit
        inputs = torch.cat([_t],axis=1)
        ...
        # output to mean
        mu_x1 = output[:,0].view(-1,1)*0.0 + a_0
        mu_x2 = output[:,1].view(-1,1)*cov_0[1,1]**0.5*_t + lam_0

        # output to cov matrix
        L_11  = output[:,2].view(-1,1)
        L_12  = output[:,3].view(-1,1)
        L_21  = output[:,4].view(-1,1)
        L_22  = output[:,5].view(-1,1)
        d     = output[:,6].view(-1,1)
        # normalize such that d**2 + L_11**2 + L_12**2 = 1
        norm_factor = torch.sqrt(d**2 + L_11**2 + L_12**2).detach()
        L_11 = L_11 / norm_factor
        L_12 = L_12 / norm_factor
        d = d / norm_factor
        cov_11 = (d**2 + L_11**2 + L_12**2)*cov_0[0,0]
        cov_12 = (L_11*L_21 + L_12*L_22)*(cov_0[0,0] * cov_0[1,1])**0.5
        cov_21 = cov_12
        cov_22 = (d**2 + L_21**2 + L_22**2)*cov_0[1,1]
        # # return p(x; mean, cov)
        cov_tensor = torch.cat([cov_11, cov_12, cov_21, cov_22], axis=1)
        cov_matrices = cov_tensor.view(-1, 2, 2)
        
        # cov_matrix = torch.from_numpy(cov_0)
        # # cov_matrices = cov_matrix.unsqueeze(0).expand(len(t), -1, -1)
        # cov_11 = (output[:,2]*0.0).view(-1,1) + cov_matrix[0,0]
        # cov_12 = (output[:,2]*0.0).view(-1,1)
        # cov_21 = cov_12
        # cov_22 = (output[:,2]**2).view(-1,1)*cov_matrix[1,1]
        # # # return p(x; mean, cov)
        # cov_tensor = torch.cat([cov_11, cov_12, cov_21, cov_22], axis=1)
        # cov_matrices = cov_tensor.view(-1, 2, 2)

        # return p(x; mu, cov)
        inverted_matrices = torch.linalg.inv(cov_matrices)
        x_diff = torch.cat([_x1-mu_x1, _x2-mu_x2], axis=1)
        x_diff_reshape = x_diff.unsqueeze(2)
        invcov_times_x_diff = torch.bmm(inverted_matrices, x_diff_reshape)
        exponent = torch.bmm(x_diff_reshape.transpose(1,2), invcov_times_x_diff).squeeze(2)
        determinants = torch.linalg.det(cov_matrices).view(-1,1)
        pdf_value = torch.exp(-0.5*exponent)/(2*torch.pi * determinants**0.5)
        if(self.verbose):
            print("mean(t): ", mu_x1[0,:].data, mu_x2[1,:].data)
            print("cov(t):  ", cov_matrices[0,:,:].data)
            # print("d(t): ", d[0,:].data)
            # print("[debug] cov inverse(t): ", inverted_matrices)
            # print("x_diff[0,1]:", x_diff[0:5,:].data)
            # print("exponent[0,1]:", exponent[0:5,].data)
            # print("determinants[0,1]:", determinants[0:5,].data)
            # print("pdf [0,1]:", pdf_value[0:5,:].data)
            # print(pdf_value.shape, pdf_value)
            # print("[operation success]")
        return pdf_value
    def get_mean_cov(self, t):
        _t  = t/t_orbit
        inputs = torch.cat([_t],axis=1)
        layer1_out = F.softplus((self.hidden_layer1(inputs)))
        layer2_out = F.softplus((self.hidden_layer2(layer1_out)))
        ...
        cov_tensor = torch.cat([cov_11, cov_12, cov_21, cov_22], axis=1)
        cov_matrices = cov_tensor.view(-1, 2, 2)
        return np.array([mu_x1[0,:].item(),mu_x2[0,:].item()]), np.array(cov_matrices[0,:,:].detach())
...
def train_p_net(p_net, optimizer, scheduler, mse_cost_function, iterations=40000):
    ...
    for epoch in range(iterations):
        ...
        # Loss Function
        loss = mse_u + 2.0*tf*(mse_res)
        # loss = torch.log(loss)
        loss_history.append(loss.data)

        # Save the min loss model
        if(loss.data < 0.95*min_loss): # if use log loss (1.05), else 0.95
...
def compare_pdf(x1_grid, x2_grid, t1, pdf, pdf_nn, pdf_linear_guassian):
    ...
    # [How can I increase the "accuracy" of Monte Carlo estimates?]
    if(t1 == 0.0):
        fig = plt.figure(figsize=(10, 8))
        ax = fig.add_subplot(111, projection='3d')
...
def compute_mean_cov(x1_grid, x2_grid, pdf, debug_text):
    ...
    if(debug_text == "pdf_nn"):
        ...
        print("[debug] pdf_nn p_sum: ", p_sum, dx1, dx2)
    #     if(p_sum > 1.0):
...
def main():
    # p_test = p_multi_normal([8000, 0.363555], mean=[7999.9575, 0.0002], cov=cov_0)
    # print(p_test)
    # dx = np.array([8000-7999.9575, 0.36355-0.0002])
    # print(dx)
    # determinants = np.linalg.det(cov_0)
    # print(determinants)
    # exponent = np.matmul(dx.T, np.matmul(np.linalg.inv(cov_0), dx))
    # p_test = np.exp(-0.5*exponent)/(2*np.pi*determinants**0.5)
    # print(p_test)
    # return
