...
def train_pinngmm_exactic(p_net, config, beta_0=0.):
    p_net.train()
    p_net.to(device)
    x_dim = p_net.D
    constants = config["constants"]
    ...
    for epoch in range(1, iterations+1):
        ...
        if loss.item() < min_loss:
            train_time = time.time() - start_time
            if(reg_tv is not None):
                print(f"--- Save Epoch: {epoch}, Loss: {loss.item():.4e}, Res: {mse_res.item():.4e} & {mse_tv.item():.4e}, Beta: {beta:.2f} ---")
            else:
                print(f"--- Save Epoch: {epoch}, Loss: {loss.item():.4e}, Res: {mse_res.item():.4e}, Beta: {beta:.2f} ---")
            best_epoch = epoch
            best_state = copy.deepcopy(p_net.state_dict())
            if save_path is not None:
                torch.save({
...
def train_pinn_error_test(networks, config):
    """
    Improved training loop for the error network e1_net.
    Uses same structure as train_pinn_sol_v0_scaled_improved.
    """
...
def train_pinn_error_test_ic(networks, config):
    """
    Improved training loop for the error network e1_net.
    Uses same structure as train_pinn_sol_v0_scaled_improved.
    """
    ...
    for epoch in range(1, iterations+1):
        ...
        if loss.item() < 0.95 * min_loss:
            ...
            #       f"IC={mse_ic.item():.4e}, Res={mse_res.item():.4e}, Beta={beta:.2f} ---")
            print(f"--- Save Epoch {epoch}, Loss={loss.item():.4e}, "
                  f"Res={mse_res.item():.4e}, Beta={beta:.2f} ---")
            # Res={mse_res.item():.4f}
