...
def generate_sobol_points_1d(N_points, x_l, x_u):
    """
    Generate N_points in [x_l, x_u] using a Sobol sequence.
    """
    sobol_engine = torch.quasirandom.SobolEngine(dimension=1, scramble=True)
    # Draw points in [0,1]
    x_tensor = sobol_engine.draw(N_points)  # shape: [N_points, 1]
    # Scale to [x_l, x_u]
    x = x_l + (x_u - x_l) * x_tensor.squeeze().numpy()
    return np.sort(x)  # sort the points


def test_1D_target_sobol():
    # Domain and discretization parameters.
    x_l = -5.0       # lower bound of x
    x_u = 5.0        # upper bound of x
    N_points = 201   # number of Sobol sample points
    x = generate_sobol_points_1d(N_points, x_l, x_u)
    n = len(x)
    
    # Compute an approximate dx (here we use uniform weights as an approximation).
    dx = (x_u - x_l) / N_points
    ...
    # Compute the normal pdf over the sampled domain.
    p0 = norm.pdf(x, loc=mu, scale=sigma)
    # Normalize p0 so that ∑ p0(x_j)*dx = 1.
    p0 = p0 / (np.sum(p0) * dx)
    print("[check] sum of p0:", np.sum(p0) * dx)
    
    # Allowed deviation.
    B = 0.1  # Adjust as needed
    
    # Target of interest (for example, the mass in [0.0, 2.0]).
    ...
    # Define the subset indices (i.e., those x between the two target values).
    sub_idx = np.where((x >= x_subset[0]) & (x <= x_subset[1]))[0]
    
    # Decision variable: p, representing the probability at each sample point.
    p = cp.Variable(n)
    ...
    # Add smoothness constraints (second difference penalty).
    c1 = 0.3
    for i in range(1, n-1):
        constraints += [ (p[i+1] - 2*p[i] + p[i-1]) / (dx*dx) <= c1,
                         -(p[i+1] - 2*p[i] + p[i-1]) / (dx*dx) <= c1]
    
    # Objective: maximize the total probability mass over the target subset.
    ...
    np.testing.assert_allclose(np.sum(p.value) * dx, 1, atol=1e-6,
    ...
    print("Test passed: the optimal distribution p satisfies the constraints.")
    ...
    plt.title("Optimal Probability Distribution $p$\n $Pr(X) =$ "+str(np.round(true_Pr,3))+", $Pr(S) =$ "+str(np.round(max_Pr, 3)))
    plt.legend()
    plt.grid(True)
    plt.show()
...
def test_1D_target():
    ...
    # Allowed deviation.
    B = 0.05  # Adjust as needed (ensure feasibility with the chosen p0)
    ...
    start_t = time.time()
    result = prob.solve()
    print("cvx solve time: ", time.time()-start_t)
    ...
    plt.plot(x, p.value, label=r"$p_{LP}$", marker="o", markersize=4, color="blue")
    plt.xlabel("x")
    plt.ylabel("Probability Density")
    plt.title(r"True $\mathbb{P}(X^'_{tar})$= "+str(np.round(true_Pr,3))+r", Max $\mathbb{P}_{LP}(X^'_{tar})$="+str(np.round(max_Pr, 3)))
    plt.legend()
    plt.grid(True)
    plt.tight_layout(pad=0.2)
    fig.savefig("figs/1d_lp.pdf", format='pdf')
    plt.show()
...
def test_1D_moment():
    ...
    # Objective: maximize momment
    objective = cp.Maximize(cp.sum( cp.multiply(p,x)) * dx)
    ...
    # --- Verification ---
    # Compute maximum possible mass on x_sub given the upper bound at each point.
    true_moment = np.sum(p0 * x)*dx
    max_moment =  np.sum(p.value * x) * dx
    print("true, max moments: ", true_moment, max_moment)
    # Assert that the total probability mass equals 1.
    np.testing.assert_allclose(
        np.sum(p.value) * dx, 
        1, 
...
def test_optimize_probability_mass_multivariate_normal_p0_4d_sobol():
    # Instead of a grid, we specify bounds for each dimension.
    bounds = np.array([
        [-5.0, 5.0],  # x bounds
        [-5.0, 5.0],  # y bounds
        [-5.0, 5.0],  # z bounds
    ...
    # Set the number of Sobol sample points.
    N_points = 2000
    samples = generate_sobol_points_4d(N_points, bounds)  # shape: [N_points, 4]
    
    # Compute the total volume of the domain.
    volume = np.prod(bounds[:,1] - bounds[:,0])
    # Integration weight for each sample is approximated as uniform.
    weight = volume / N_points
    
    # Define the "baseline" 4D multivariate normal parameters.
    mu = np.zeros(4)
    sigma = np.eye(4)
    rv = multivariate_normal(mean=mu, cov=sigma)
    
    # Evaluate p0 at each sample point.
    p0 = rv.pdf(samples)  # shape: [N_points]
    # Normalize p0 so that its approximate integral equals 1.
    p0 = p0 / (np.sum(p0) * weight)
    print("[check] Integrated p0 (should be 1):", np.sum(p0) * weight)
    
    # Allowed deviation.
    B = 0.001
    
    # Set a target region in 4D.
    # For this example, we require x and y to be in [1.0, 1.1], and no restriction on z and w.
    target_region = np.array([
        [-3.0, 1.0],   # x bounds
        [1.0, 3.0],   # y bounds
        [-5.0, 5.0],  # z bounds (full domain)
        [-5.0, 5.0]   # w bounds (full domain)
    ])
    
    # Create a mask for samples that fall into the target region.
    mask = np.ones(N_points, dtype=bool)
    for dim in range(4):
        mask = mask & (samples[:, dim] >= target_region[dim, 0]) & (samples[:, dim] <= target_region[dim, 1])
    
    # The baseline and the mask are defined on the vector of N_points.
    p0_flat = p0  # already vector of shape [N_points]
    mask_flat = mask.astype(float)  # convert boolean to 0/1 for multiplication.
    
    # Decision variable: p_vec (a vector of length N_points) for the unknown probability density.
    p_vec = cp.Variable(N_points)
    
    # Define the constraints:
    constraints = [
        cp.sum(cp.multiply(p_vec, weight)) == 1,  # total integrated probability equals 1
        p_vec >= 0,                              # non-negativity
        p_vec >= p0_flat - B,                      # lower bound constraint
        p_vec <= p0_flat + B                       # upper bound constraint
    ]
    
    # For this 4D application, adding smoothness constraints is more complex because the samples do 
    # not lie on a structured grid. One common approach is to use a k-nearest neighbor method or 
    # graph Laplacian regularization. For simplicity here, we will omit the smoothness penalty.
    #
    # (If desired, one may build an approximate difference operator using the geometry 
    # of the sample points.)
    
    # Objective: maximize the total integrated probability mass over the target region.
    objective = cp.Maximize(cp.sum(cp.multiply(p_vec, mask_flat)) * weight)
    
    # Solve the LP.
    prob = cp.Problem(objective, constraints)
    result = prob.solve()
    
    true_Pr = np.sum(p0_flat[mask]) * weight
    max_Pr  = np.sum(p_vec.value[mask]) * weight
    print("True probability mass in target region:", true_Pr)
    print("Optimal objective mass in target region:", max_Pr)
    print("Total integrated mass (should be 1):", np.sum(p_vec.value) * weight)
    
    # For visualization, we create a 2D scatter of the first two dimensions, coloring the samples by p0 and p.
    # plt.figure(figsize=(10, 6))
    # plt.subplot(1, 2, 1)
    # sc0 = plt.scatter(samples[:, 0], samples[:, 1], c=p0_flat, cmap="viridis", marker="o")
    # plt.colorbar(sc0)
    # plt.title("Baseline $p_0$ (first two dims)")
    # plt.xlabel("x")
    # plt.ylabel("y")
    
    # plt.subplot(1, 2, 2)
    # sc1 = plt.scatter(samples[:, 0], samples[:, 1], c=p_vec.value, cmap="viridis", marker="o")
    # plt.colorbar(sc1)
    # plt.title("Optimal $p$ (first two dims)")
    # plt.xlabel("x")
    # plt.ylabel("y")
...
def test_function_opt_1D():
    ...
    plt.grid(True)
    plt.tight_layout(pad=0.2)
    fig.savefig("figs/1d_fo.pdf", format='pdf')
    plt.show()
...
if __name__ == '__main__':
    ...
    # test_1D_target_sobol()

    # test_optimize_probability_mass_multivariate_normal_p0_4d_vectorized()
    # test_optimize_probability_mass_multivariate_normal_p0_4d_sobol()
    # test_1D_moment()
