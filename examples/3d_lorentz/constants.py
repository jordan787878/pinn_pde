...
class MyConstantsLorentz:
    ...
    _RHO =   np.float32(16.0)
    _L = np.float32(np.diag([0.05, 0.05, 0.05]))
    _MEAN_I   = np.float32([0.1, 0.3, 1.0])
    _COV_I    = np.float32(np.diag([0.3, 0.3, 0.3]))
    _X_RANGE = np.float32(np.array([[-2.0, 3.0],
                                    [-3.0, 4.0],
                                    [-1.0, 3.0]]))
    _TI = np.float32(0.0)
    _TF = np.float32(0.1)
    _T_SPAN = np.float32(np.array([_TI, 0.25*_TF, 0.5*_TF, _TF]))
    ...
    def p_sol_monte(self, t=0.0, linespace_num=81, stat_sample=100000000):
        ...
        for i in tqdm(range(stat_sample), desc="Processing samples"):
            x = X[i,:]
            for t in t_span:
                w = np.random.normal(0, np.sqrt(dtt), 3)
